...
#         gaussian_mean = .0
#         W1 = np.random.normal(gaussian_mean, weight_scale, (input_dim, hidden_dim))
#         b1 = np.zeros((hidden_dim,))
#         W2 = np.random.normal(gaussian_mean, weight_scale, (hidden_dim, num_classes))
...
#         gaussian_mean = .0
#         self.params['W1'] = np.random.normal(gaussian_mean, weight_scale, (input_dim, hidden_dims[0]))
...
#             self.params['W{}'.format(n + 1)] = np.random.normal(gaussian_mean, weight_scale, (hidden_dims[n-1], hidden_dims[n]))
#             self.params['b{}'.format(n + 1)] = np.zeros((hidden_dims[n],))
#             if self.normalization != None:
#                 self.params['gamma{}'.format(n + 1)] = np.ones((hidden_dims[n],))
#                 self.params['beta{}'.format(n + 1)] = np.zeros((hidden_dims[n],))
        
#         self.params['W{}'.format(self.num_layers)] = np.random.normal(gaussian_mean, weight_scale, (hidden_dims[self.num_layers-2], num_classes))
...
#             if self.normalization == None:
#                 out, cache["{}".format(n+1)] = affine_relu_forward(out, self.params['W{}'.format(n+1)], self.params['b{}'.format(n+1)])
#             elif self.normalization == "batchnorm":
#                 out, cache["{}".format(n+1)] = affine_bn_relu_forward(out, self.params['W{}'.format(n+1)], self.params['b{}'.format(n+1)], self.params['gamma{}'.format(n+1)], self.params['beta{}'.format(n+1)], self.bn_params[n])
#             elif self.normalization == "layernorm":
#                 out, cache["{}".format(n+1)] = affine_ln_relu_forward(out
#                 , self.params['W{}'.format(n+1)], self.params['b{}'.format(n+1)], self.params['gamma{}'.format(n+1)], self.params['beta{}'.format(n+1)], self.bn_params[n])
#             if self.use_dropout:
#                 out, dropout_cache["{}".format(n+1)] = dropout_forward(out, self.dropout_param)
#         scores, cache["{}".format(self.num_layers)] = affine_forward(out, self.params['W{}'.format(self.num_layers)], self.params['b{}'.format(self.num_layers)])
...
#             loss += 0.5 * self.reg * (np.sum(self.params['W{}'.format(n + 1)]**2))
#         dh, dW, db = affine_backward(dscores, cache['{}'.format(self.num_layers)])
#         dW += self.reg * self.params['W{}'.format(self.num_layers)]
#         grads['W{}'.format(self.num_layers)] = dW
#         grads['b{}'.format(self.num_layers)] = db

#         for n in reversed(range(self.num_layers - 1)):
#             if self.use_dropout:
#                 dh = dropout_backward(dh, dropout_cache['{}'.format(n+1)])
#             if self.normalization == "batchnorm":
#                 dh, dW, db, dgamma, dbeta = affine_bn_relu_backward(dh, cache['{}'.format(n+1)])
#                 grads['gamma{}'.format(n+1)] = dgamma
#                 grads['beta{}'.format(n+1)] = dbeta 
#             if self.normalization == "layernorm":
#                 dh, dW, db, dgamma, dbeta = affine_ln_relu_backward(dh, cache['{}'.format(n+1)])
#                 grads['gamma{}'.format(n+1)] = dgamma
#                 grads['beta{}'.format(n+1)] = dbeta 
#             else:
#                 dh, dW, db = affine_relu_backward(dh, cache['{}'.format(n+1)])
#             dW += self.reg * self.params['W{}'.format(n+1)]
#             grads['W{}'.format(n+1)] = dW
#             grads['b{}'.format(n+1)] = db
#         # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
#         ############################################################################
#         #                             END OF YOUR CODE                             #
#         ############################################################################
