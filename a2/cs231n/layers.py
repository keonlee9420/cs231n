...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == "train":
        ...
        # batch_variance = np.var(x, axis=0) # (D,)
        # normalized_x = (x - batch_mean) / np.sqrt(batch_variance + eps)
        # scaled_and_shifted = gamma * normalized_x + beta
        ...
        # cache = (batch_mean, batch_variance, eps, normalized_x, N, beta, gamma, x)
...
def batchnorm_backward(dout, cache):
    ...
    # (batch_mean, batch_variance, eps, normalized_x, N, beta, gamma, x) = cache
    # denominator = 1/(np.sqrt(batch_variance + eps))
    # numerator = (x - batch_mean)

    # dnormalized_x = dout * gamma
    # dbatch_variance = np.sum(dnormalized_x * numerator * (-1/2) * (denominator**3), axis=0)
    # dbatch_mean = np.sum(dnormalized_x * (-1) * denominator, axis=0) + dbatch_variance * (1/N) * np.sum((-2) * numerator, axis=0)
    # dx = dnormalized_x * denominator + dbatch_variance * (2/N) * (numerator) + dbatch_mean * (1/N)
...
# layernorm follows https://github.com/jariasf/CS231n/blob/master/assignment2/cs231n/layers.py
...
def layernorm_backward(dout, cache):
    ...
    # (batch_mean, batch_variance, eps, normalized_x, N, beta, gamma, x) = cache
    # dgamma= np.sum(normalized_x * dout.T, axis=1)
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    for j in range(H_out):
        for i in range(W_out):
            # print(j*stride, j*stride+HH, i*stride, i*stride+WW)
            receptive_field = x_pad[:,:,j*stride:j*stride+HH, i*stride:i*stride+WW] # (N, C, HH, WW)
            # print(j, i, receptive_field.shape)
            for f in range(F):
                w_f = w[f, :, :, :] # (C, HH, WW)
                # print("w_f:", w_f)
                # print("receptive_field:", receptive_field)
                # print("w_f * receptive_field:", w_f * receptive_field)
                convolved = (w_f * receptive_field).sum(axis=(1,2,3)) # (N,)
                # print((w_f * receptive_field)[0].sum(), (w_f * receptive_field)[1].sum())
...
def conv_backward_naive(dout, cache):
    ...
    dx_pad = np.zeros_like(x_pad) # (N, C, H_p, W_p)
    ...
    for j in range(H_out):
        for i in range(W_out):
            receptive_field = x_pad[:,:,j*stride:j*stride+HH, i*stride:i*stride+WW] # (N, C, HH, WW)
            for f in range(F):
                dw[f, :, :, :] += (receptive_field \
                    * np.expand_dims(dout[:,f,j,i], (1,2,3))).sum(axis=(0,)) # (N, C, HH, WW) * (N, 1, 1, 1) -> (C, HH, WW)
                dx_pad[:,:,j*stride:j*stride+HH, i*stride:i*stride+WW] += \
                    np.tile(np.expand_dims(w[f,:,:,:], axis=0), (N,1,1,1)) * np.expand_dims(dout[:,f,j,i], (1,2,3)) # (N, C, HH, WW) * (N, 1, 1, 1) -> (N, C, HH, WW)
...
def max_pool_forward_naive(x, pool_param):
    ...
    for j in range(H_out):
        for i in range(W_out):
            receptive_field = x[:,:,j*stride:j*stride+pool_height, i*stride:i*stride+pool_width] # (N, C, pool_height, pool_width)
            out[:,:,j,i] = np.amax(receptive_field, axis=(2,3)) # (N, C,)
...
def max_pool_backward_naive(dout, cache):
    ...
    for j in range(H_out):
        for i in range(W_out):
            ...
            rf_max = receptive_field == np.expand_dims(np.amax(receptive_field, axis=(2,3)), (2,3))
            # print(rf_max)
            # print(np.ones_like(receptive_field) * rf_max * np.expand_dims(dout[:,:,j,i], (2,3)))
            dx_rf = np.ones_like(receptive_field) * rf_max * np.expand_dims(dout[:,:,j,i], (2,3))# (N, C, pool_height, pool_width)
            dx[:,:,j*stride:j*stride+pool_height, i*stride:i*stride+pool_width] += dx_rf
...
# spatial batchnorm follows https://github.com/williamchan/cs231-assignment2/blob/master/cs231n/layers.py
...
# spatial groupnorm follows https://www.jianshu.com/p/aaeb9cd4d70c
